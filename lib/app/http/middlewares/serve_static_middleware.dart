...
class ServeStaticMiddleware extends Middleware {
  static const supportedExtensions = ['js', 'ico', 'json', 'html', 'png', 'otf', 'ttf'];
  ...
  handle(Request req, Response res, NextFunction next) async {
    ...
    if (!supportedExtensions.contains(lastPart) || ![HTTPMethod.GET, HTTPMethod.GET].contains(req.method)) {
      return next();
    }
    ...
    if (!await requestedFile.exists()) {
      return next(res.status(HttpStatus.notFound).end());
    }
